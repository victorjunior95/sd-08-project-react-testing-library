...
describe('Tests the PokemonDetails component', () => {
  ...
  it('Must contain a <p>summary of the selected Pokémon</p>', () => {
    ...
    expect(
      getByText(/It can freely detach its jaw to swallow/i),
    ).toBeInTheDocument();
  });
});
...
describe('There must be a section containing location Pokémon map', () => {
  ...
  it('Must be shown each name location and its image map', () => {
    ...
    const mapImg = getByAltText(/Snorlax location/i);
    expect(mapImg.src).toBe(
      'https://cdn.bulbagarden.net/upload/5/54/Kanto_Vermilion_City_Map.png',
    );
    const mapLocation = getByText(/Kanto Vermillion City/i);

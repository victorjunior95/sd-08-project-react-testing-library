...
describe('Pagina inicial', () => {
  it('As informações detalhadas do Pokémon selecionado são mostradas na tela.', () => {
    const { history, getByRole, getByText, getByTestId } = renderWithRouter(<App />);
    ...
    const sumaryText = getByText('This intelligent Pokémon roasts hard berries with electricity to make them tender enough to eat.');
  ...
  it('Existe na página uma seção com os mapas contendo as localizações do pokémon', () => {
    const { history, getByRole, getByText, getByTestId, getAllByAltText, getAllByRole } = renderWithRouter(<App />);
    const url = history.location.pathname;
    const link = getByRole('link', {
      name: /More Details/i,
    });
